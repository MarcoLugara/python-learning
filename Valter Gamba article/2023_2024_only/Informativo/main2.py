...
# =============================================================================
# COMPANY-LEVEL ANALYSIS (ADAPTED FOR 3-FIELD DATASET)
# =============================================================================

def analyze_top_performers(df, year='2024'):
    """Identify all companies with 2/3 or 3/3 fields completed in a given year."""
    ...
    # Get companies with 2 or 3 fields completed (2/3 or 3/3 for 3-field dataset)
    top_performers = df[['Name', 'ateco_letter', total_sum_col]].copy()
    top_performers = top_performers[top_performers[total_sum_col] >= 2]
    top_performers = top_performers.sort_values(by=total_sum_col, ascending=False)

    # Calculate percentage completion (out of 3 fields)
    top_performers['Percentage'] = (top_performers[total_sum_col] / 3) * 100
...
def analyze_best_improvers(df):
    """Identify all companies that improved by 2 or 3 points from 2023 to 2024."""
    ...
    required_cols = ['Name', 'ateco_letter', 'total_sum_2023', 'total_sum_2024']
    ...
    improvers = df[['Name', 'ateco_letter', 'total_sum_2023', 'total_sum_2024']].copy()
    improvers['Improvement'] = improvers['total_sum_2024'] - improvers['total_sum_2023']

    # Get companies with improvement of 2 or 3 points (for 3-field dataset)
    top_improvers = improvers[improvers['Improvement'].isin([2, 3])]
    top_improvers = top_improvers.sort_values(by='Improvement', ascending=False)

    # Calculate percentage improvement (out of 3 fields)
    top_improvers['Percentage_Improvement'] = (top_improvers['Improvement'] / 3) * 100
...
# =============================================================================
# COMPANY-LEVEL ANALYSIS EXECUTION (FOR 3-FIELD DATASET)
# =============================================================================
...
print("\n" + "="*60)
print("EXCELLENT PERFORMING COMPANIES IN 2024 (2/3 or 3/3 fields)")
print("="*60)
if not top_performers_2024.empty:
    print(f"Total companies with excellent performance: {len(top_performers_2024)}")
    print(top_performers_2024[['Name', 'ateco_letter', 'total_sum_2024', 'Percentage']].to_string())
else:
    print("No company achieved 2/3 or 3/3 fields in 2024")

print("\n" + "="*60)
print("BEST IMPROVERS (2023 to 2024, improved by 2 or 3 points)")
print("="*60)
if not top_improvers_2023_2024.empty:
    print(f"Total companies with significant improvement: {len(top_improvers_2023_2024)}")
    print(top_improvers_2023_2024[['Name', 'ateco_letter', 'total_sum_2023', 'total_sum_2024', 'Improvement', 'Percentage_Improvement']].to_string())
else:
    print("No company improved by 2 or 3 points from 2023 to 2024")

# =============================================================================
# ENHANCED PDF REPORT (UPDATED WITH COMPANY-LEVEL ANALYSIS)
# =============================================================================
...
def create_pdf_report():
    ...
    if has_company_names and local_top_performers_2024 is not None and not local_top_performers_2024.empty:
        # Subsection 7.1: Top Performers in 2024
        story.append(Paragraph("7.1 Excellent Performing Companies in 2024", heading2_style))
        story.append(Paragraph("Companies that achieved 2/3 or 3/3 fields completed in 2024:", normal_style))
        story.append(Paragraph(f"<b>Total companies with excellent performance:</b> {len(local_top_performers_2024)}",
                               normal_style))

        # Count by category
        perfect_scores = len(local_top_performers_2024[local_top_performers_2024['total_sum_2024'] == 3])
        excellent_scores = len(local_top_performers_2024[local_top_performers_2024['total_sum_2024'] == 2])
        story.append(Paragraph(f"<b>Perfect scores (3/3):</b> {perfect_scores} companies", normal_style))
        story.append(Paragraph(f"<b>Excellent scores (2/3):</b> {excellent_scores} companies", normal_style))

        story.append(Spacer(1, 10))

        # Create table for top performers
        top_performers_data = [['Company Name', 'Sector', 'Fields Completed', 'Completion %']]

        for idx, row in local_top_performers_2024.iterrows():
            top_performers_data.append([
                str(row['Name']),
                str(row['ateco_letter']),
                f"{int(row['total_sum_2024'])}/3",
                f"{row['Percentage']:.1f}%"
            ])

        # Create table
        top_performers_table = Table(top_performers_data, colWidths=[200, 80, 100, 100])
        ...
        story.append(Paragraph("No company achieved 2/3 or 3/3 fields in 2024.", normal_style))

    story.append(Spacer(1, 20))

    if has_company_names and local_top_improvers_2023_2024 is not None and not local_top_improvers_2023_2024.empty:
        # Subsection 7.2: Best Improvers 2023-2024
        story.append(Paragraph("7.2 Most Improved Companies (2023 to 2024)", heading2_style))
        story.append(Paragraph("Companies that improved by 2 or 3 points from 2023 to 2024:", normal_style))
        story.append(
        ...
        improvement_2 = len(local_top_improvers_2023_2024[local_top_improvers_2023_2024['Improvement'] == 2])

        story.append(Paragraph(f"<b>Maximum improvement (+3 points):</b> {improvement_3} companies", normal_style))
        story.append(Paragraph(f"<b>Major improvement (+2 points):</b> {improvement_2} companies", normal_style))

        story.append(Spacer(1, 10))

        # Create table for top improvers
        top_improvers_data = [['Company Name', 'Sector', '2023 Fields', '2024 Fields', 'Improvement', '% Change']]

        for idx, row in local_top_improvers_2023_2024.iterrows():
            top_improvers_data.append([
                str(row['Name']),
                str(row['ateco_letter']),
                f"{int(row['total_sum_2023'])}/3",
                f"{int(row['total_sum_2024'])}/3",
        ...
        # Create table
        top_improvers_table = Table(top_improvers_data, colWidths=[180, 70, 70, 70, 80, 80])
    ...
    if has_company_names:
        ...
        if local_top_performers_2024 is not None and not local_top_performers_2024.empty:
            ...
            perfect_scorers = len(local_top_performers_2024[local_top_performers_2024['total_sum_2024'] == 3])
            if perfect_scorers > 0:
                if perfect_scorers == 1:
                    company_name = local_top_performers_2024[local_top_performers_2024['total_sum_2024'] == 3].iloc[0][
                        'Name']
                    sector = local_top_performers_2024[local_top_performers_2024['total_sum_2024'] == 3].iloc[0][
                        'ateco_letter']
                    insights_company.append(f"• Only one company achieved perfect score (3/3): {company_name} (Sector: {sector})")
                else:
                    insights_company.append(f"• {perfect_scorers} companies achieved perfect scores (3/3 fields)")
            ...
            insights_company.append(
                f"• {total_excellent} companies ({pct_excellent:.1f}%) achieved excellent performance (2/3 or 3/3)")
        ...
        if local_top_improvers_2023_2024 is not None and not local_top_improvers_2023_2024.empty:
            ...
            zero_to_excellent = local_top_improvers_2023_2024[
                ...
                (local_top_improvers_2023_2024['total_sum_2024'] >= 2)
                ]
            if len(zero_to_excellent) > 0:
                insights_company.append(
                    f"• {len(zero_to_excellent)} companies went from 0 fields to excellent performance (2+ fields)")

            # Insight 4: Biggest turnaround
            max_improvement = local_top_improvers_2023_2024['Improvement'].max()
            if max_improvement == 3:
                companies_with_max = local_top_improvers_2023_2024[local_top_improvers_2023_2024['Improvement'] == 3]
                if len(companies_with_max) > 0:
                    if len(companies_with_max) == 1:
                        company_name = companies_with_max.iloc[0]['Name']
                        sector = companies_with_max.iloc[0]['ateco_letter']
                        insights_company.append(
                            f"• {company_name} (Sector: {sector}) made the most dramatic improvement: from 0 to 3 fields")
                    else:
                        insights_company.append(
                            f"• {len(companies_with_max)} companies made the maximum possible improvement (0 to 3 fields)")
...
if not top_performers_2024.empty:
    ...
    excellent_scores = len(top_performers_2024[top_performers_2024['total_sum_2024'] == 2])
    print(f"  - Companies with excellent performance (2/3 or 3/3): {len(top_performers_2024)}")
    print(f"    Perfect scores (3/3): {perfect_scores} companies")
    print(f"    Excellent scores (2/3): {excellent_scores} companies")
    if perfect_scores > 0:
        perfect_companies = top_performers_2024[top_performers_2024['total_sum_2024'] == 3]['Name'].tolist()
        perfect_sectors = top_performers_2024[top_performers_2024['total_sum_2024'] == 3]['ateco_letter'].tolist()
        print(f"    Perfect companies: " + ", ".join([f"{name} ({sector})" for name, sector in zip(perfect_companies[:3], perfect_sectors[:3])]) +
...
if not top_improvers_2023_2024.empty:
    ...
    improvement_2 = len(top_improvers_2023_2024[top_improvers_2023_2024['Improvement'] == 2])
    print(f"  - Companies with significant improvement (2+ points): {len(top_improvers_2023_2024)}")
    print(f"    Maximum improvement (+3): {improvement_3} companies")
    print(f"    Major improvement (+2): {improvement_2} companies")
    if improvement_3 > 0:
        massive_improvers = top_improvers_2023_2024[top_improvers_2023_2024['Improvement'] == 3]['Name'].tolist()
        massive_sectors = top_improvers_2023_2024[top_improvers_2023_2024['Improvement'] == 3]['ateco_letter'].tolist()
        print(f"    Companies that improved by 3 points: " + ", ".join([f"{name} ({sector})" for name, sector in zip(massive_improvers[:3], massive_sectors[:3])]) +
