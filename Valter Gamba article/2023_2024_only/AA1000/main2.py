...
if 'Name' not in df.columns:
    ...
    possible_name_columns = ['Name', 'name', 'Company Name', 'company_name', 'Company', 'company']
    name_col = None
    ...
    if name_col:
        ...
        # Create a placeholder Name column if not found
        print("Warning: 'Name' column not found. Creating placeholder names.")
        df['Name'] = ['Company_' + str(i) for i in range(len(df))]
...
def analyze_top_companies():
    """Analyze and return top performing companies and best improvers."""

    # Ensure we have the necessary columns
    required_cols = ['Name', 'AA1000_2023', 'AA1000_2024']
    ...
    # Create a copy for company analysis
    company_df = df[['Name', 'ateco_letter', 'AA1000_2023', 'AA1000_2024']].copy()

    # Calculate improvement (change from 2023 to 2024)
    company_df['improvement'] = company_df['AA1000_2024'] - company_df['AA1000_2023']

    # 1. Best performing companies in 2024 (highest sums)
    # Since AA1000 is binary (0 or 1), best performers are those with AA1000_2024 = 1
    best_performers_2024 = company_df[company_df['AA1000_2024'] == 1].copy()

    # Sort by AA1000_2024 (already all 1s) and then by name
    best_performers_2024 = best_performers_2024.sort_values(['AA1000_2024', 'Name'], ascending=[False, True])

    # Add ranking
    best_performers_2024['Rank'] = range(1, len(best_performers_2024) + 1)

    # 2. Best improvers between 2023 and 2024
    # Companies that improved from 0 in 2023 to 1 in 2024 (improvement = 1)
    best_improvers = company_df[company_df['improvement'] == 1].copy()

    # Sort by improvement (all are 1) and then by name
    best_improvers = best_improvers.sort_values(['improvement', 'Name'], ascending=[False, True])

    # Add ranking
    best_improvers['Rank'] = range(1, len(best_improvers) + 1)

    # 3. Companies with perfect adoption (AA1000 in both years)
    perfect_adopters = company_df[
        (company_df['AA1000_2023'] == 1) &
        (company_df['AA1000_2024'] == 1)
        ].copy()

    perfect_adopters = perfect_adopters.sort_values('Name')
    perfect_adopters['Rank'] = range(1, len(perfect_adopters) + 1)

    # 4. Companies with no adoption in either year
    no_adopters = company_df[
        (company_df['AA1000_2023'] == 0) &
        (company_df['AA1000_2024'] == 0)
        ].copy()

    no_adopters = no_adopters.sort_values('Name')
    no_adopters['Rank'] = range(1, len(no_adopters) + 1)

    # Summary statistics
    summary_stats = {
        'total_companies': len(company_df),
        'adopters_2024': len(best_performers_2024),
        'pct_adopters_2024': (len(best_performers_2024) / len(company_df) * 100) if len(company_df) > 0 else 0,
        'improvers_23_24': len(best_improvers),
        'pct_improvers': (len(best_improvers) / len(company_df) * 100) if len(company_df) > 0 else 0,
        'perfect_adopters': len(perfect_adopters),
        'no_adopters': len(no_adopters),
        'regressed_companies': len(company_df[company_df['improvement'] == -1])  # Went from 1 to 0
    }

    return {
        'best_performers_2024': best_performers_2024,
        'best_improvers': best_improvers,
        'perfect_adopters': perfect_adopters,
        'no_adopters': no_adopters,
        'summary_stats': summary_stats,
        'all_companies': company_df
    }
...
company_analysis = analyze_top_companies()
...
def create_pdf_report():
    ...
    if company_analysis:
        ...
        story.append(Paragraph("This section identifies individual company performance and improvement.", normal_style))
        ...
        summary_company_data = [
            ['Metric', 'Value'],
            ['Total Companies', f"{summary_stats['total_companies']:,}"],
            ['AA1000 Adopters in 2024', f"{summary_stats['adopters_2024']:,}"],
            ['AA1000 Adoption Rate in 2024', f"{summary_stats['pct_adopters_2024']:.1f}%"],
            ['Best Improvers (2023-2024)', f"{summary_stats['improvers_23_24']:,}"],
            ['Improvement Rate', f"{summary_stats['pct_improvers']:.1f}%"],
            ['Perfect Adopters (Both Years)', f"{summary_stats['perfect_adopters']:,}"],
            ['Companies with No Adoption', f"{summary_stats['no_adopters']:,}"],
            ['Companies that Regressed (1→0)', f"{summary_stats['regressed_companies']:,}"]
        ]

        summary_company_table = Table(summary_company_data, colWidths=[200, 120])
        summary_company_table.setStyle(TableStyle([
            ('BACKGROUND', (0, 0), (-1, 0), colors.HexColor('#2c3e50')),
            ('TEXTCOLOR', (0, 0), (-1, 0), colors.whitesmoke),
            ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
            ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
        ...
        story.append(summary_company_table)
        story.append(Spacer(1, 15))

        # 7.1 Best Performing Companies in 2024
        story.append(Paragraph("7.1 Best Performing Companies in 2024 (AA1000 Adopters)", heading2_style))
        story.append(Paragraph(f"These {len(company_analysis['best_performers_2024'])} companies have AA1000 in 2024:",
                               normal_style))

        best_performers = company_analysis['best_performers_2024']
        if len(best_performers) > 0:
            # Show top 30 companies to avoid making the table too long
            top_n = min(30, len(best_performers))
            story.append(Paragraph(f"Showing top {top_n} companies (sorted alphabetically):", normal_style))

            # Prepare table data
            perf_headers = ['Rank', 'Company Name', 'ATECO Sector', 'AA1000 2023', 'AA1000 2024']
            perf_data = [perf_headers]

            for idx, (_, row) in enumerate(best_performers.head(top_n).iterrows()):
                perf_data.append([
                    str(idx + 1),
                    str(row['Name'])[:40],  # Limit name length
                    str(row.get('ateco_letter', 'N/A')),
                    'Yes' if row['AA1000_2023'] == 1 else 'No',
                    'Yes' if row['AA1000_2024'] == 1 else 'No'
                ])

            perf_table = Table(perf_data, colWidths=[40, 200, 60, 80, 80])
            ...
            if len(best_performers) > top_n:
                story.append(Paragraph(f"... and {len(best_performers) - top_n} more companies with AA1000 in 2024.",
                                       normal_style))
        else:
            story.append(Paragraph("No companies with AA1000 in 2024 found.", normal_style))

        story.append(Spacer(1, 15))

        # 7.2 Best Improvers (2023-2024)
        story.append(Paragraph("7.2 Best Improvers (2023-2024)", heading2_style))
        story.append(Paragraph(
            f"These {len(company_analysis['best_improvers'])} companies improved from no AA1000 in 2023 to having AA1000 in 2024:",
            normal_style))

        best_improvers = company_analysis['best_improvers']
        if len(best_improvers) > 0:
            # Show top 30 companies
            top_n = min(30, len(best_improvers))
            story.append(Paragraph(f"Showing top {top_n} companies (sorted alphabetically):", normal_style))

            # Prepare table data
            improv_headers = ['Rank', 'Company Name', 'ATECO Sector', 'AA1000 2023', 'AA1000 2024', 'Improvement']
            improv_data = [improv_headers]

            for idx, (_, row) in enumerate(best_improvers.head(top_n).iterrows()):
                improv_data.append([
                    str(idx + 1),
                    str(row['Name'])[:40],  # Limit name length
                    str(row.get('ateco_letter', 'N/A')),
                    'No' if row['AA1000_2023'] == 0 else 'Yes',
                    'Yes' if row['AA1000_2024'] == 1 else 'No',
                    '0 → 1'
                ])

            improv_table = Table(improv_data, colWidths=[40, 180, 60, 70, 70, 70])
            ...
            story.append(improv_table)

            if len(best_improvers) > top_n:
                story.append(
                    Paragraph(f"... and {len(best_improvers) - top_n} more companies that improved.", normal_style))
        else:
            story.append(Paragraph("No companies improved from 2023 to 2024.", normal_style))

        story.append(Spacer(1, 15))

        # 7.3 Perfect Adopters
        story.append(Paragraph("7.3 Perfect Adopters (AA1000 in Both Years)", heading2_style))
        perfect_adopters = company_analysis['perfect_adopters']
        if len(perfect_adopters) > 0:
            story.append(Paragraph(
                f"{len(perfect_adopters)} companies have consistently maintained AA1000 in both 2023 and 2024:",
                normal_style))

            # Show top 20 perfect adopters
            top_n = min(20, len(perfect_adopters))
            perfect_data = [['Rank', 'Company Name', 'ATECO Sector']]

            for idx, (_, row) in enumerate(perfect_adopters.head(top_n).iterrows()):
                perfect_data.append([
                    str(idx + 1),
                    str(row['Name'])[:40],
                    str(row.get('ateco_letter', 'N/A'))
            ...
            perfect_table = Table(perfect_data, colWidths=[40, 220, 60])
            ...
            if len(perfect_adopters) > top_n:
                story.append(
                    Paragraph(f"... and {len(perfect_adopters) - top_n} more consistent adopters.", normal_style))
        else:
            story.append(Paragraph("No companies have AA1000 in both years.", normal_style))

        story.append(PageBreak())

        # Re-number the next section
        next_section_num = 8
    else:
        story.append(Paragraph("7. Company-Level Analysis", heading1_style))
        story.append(
            Paragraph("Company-level analysis could not be performed. 'Name' column may be missing from the dataset.",
                      normal_style))
        story.append(PageBreak())
        next_section_num = 8

    # Section 8: Key Findings and Insights (re-numbered)
    story.append(Paragraph(f"{next_section_num}. Key Findings and Insights", heading1_style))
    ...
    # Add company-level insights if available
    if company_analysis:
        summary_stats = company_analysis['summary_stats']
        insights.append(f"8. Top Performers: {summary_stats['adopters_2024']} companies have AA1000 in 2024.")
        insights.append(
            f"9. Best Improvers: {summary_stats['improvers_23_24']} companies adopted AA1000 between 2023-2024.")
        insights.append(
            f"10. Consistency: {summary_stats['perfect_adopters']} companies maintained AA1000 in both years.")
...
# Print company analysis summary to console
if company_analysis:
    print(f"\n✓ Company-Level Analysis Summary:")
    stats = company_analysis['summary_stats']
    print(f"  - Total companies analyzed: {stats['total_companies']:,}")
    print(f"  - AA1000 adopters in 2024: {stats['adopters_2024']:,} ({stats['pct_adopters_2024']:.1f}%)")
    print(f"  - Best improvers (2023-2024): {stats['improvers_23_24']:,} ({stats['pct_improvers']:.1f}%)")
    print(f"  - Perfect adopters (both years): {stats['perfect_adopters']:,}")
    print(f"  - Companies with no adoption: {stats['no_adopters']:,}")

    # Show top 10 companies from each category
    print(f"\n  Top 10 Best Performing Companies in 2024:")
    best_performers = company_analysis['best_performers_2024'].head(10)
    for idx, (_, row) in enumerate(best_performers.iterrows()):
        print(f"    {idx + 1}. {row['Name']} (Sector: {row.get('ateco_letter', 'N/A')})")

    print(f"\n  Top 10 Best Improvers (2023-2024):")
    best_improvers = company_analysis['best_improvers'].head(10)
    for idx, (_, row) in enumerate(best_improvers.iterrows()):
        print(f"    {idx + 1}. {row['Name']} (Sector: {row.get('ateco_letter', 'N/A')}) - Improved from 0 to 1")