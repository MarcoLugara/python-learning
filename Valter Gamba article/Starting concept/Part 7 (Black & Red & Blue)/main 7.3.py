...
def create_percentage_bar_chart(year_data, year, sector_order):
    ...
    # Add total percentage labels on top of bars (SIMPLE - no box)
    for i, (sector, total_pct) in enumerate(zip(sorted_sector_names, bottoms)):
        if total_pct > 0:
            ...
            # Add total company count below x-axis
            total_companies = sector_totals.get(sector, 0)
            ax.text(i, -y_max * 0.05, f'n={total_companies}',
                    ha='center', va='top', fontsize=8, color='gray')

    # Create a comprehensive legend (same as absolute chart)
    legend_items = []

    # First, create group headers
    for group_name in group_order:
        group_info = field_groups[group_name]
        legend_items.append((Line2D([0], [0], color='white'),
                             f"<b>{group_info['name']}:</b>"))
...
def create_pdf_report():
    ...
    story.append(
        Paragraph("Detailed analysis of field completion distributions across all sectors and years", normal_style))

    # Distribution statistics table
    dist_headers = ['Fields Completed', 'Companies (2022)', '% (2022)', 'Companies (2023)', '% (2023)',
                    'Companies (2024)', '% (2024)']
    ...
    # FIX: Correct the unpacking order - it should be (start, end, label) not (label, start, end)
    for start, end, label in ranges:  # CHANGED THIS LINE
        row = [label]
        for year in ['2022', '2023', '2024']:
    ...
    dist_table = Table(dist_data, colWidths=[80, 80, 70, 80, 70, 80, 70])
    ...
    story.append(Paragraph("Distribution Summary:", heading2_style))

    # Calculate mode safely
    mode_values = df['total_sum_2024'].mode()
    mode_str = f"{mode_values.iloc[0]}" if not mode_values.empty else 'N/A'

    story.append(Paragraph(f"• Most common completion level (mode): {mode_str} fields", bullet_style))
    story.append(Paragraph(f"• 25th percentile (Q1): {advanced_stats['yearly_stats']['2024']['q1_fields']:.1f} fields",
                           bullet_style))
    story.append(
        Paragraph(f"• 50th percentile (median): {advanced_stats['yearly_stats']['2024']['median_fields']:.1f} fields",
                  bullet_style))
    story.append(Paragraph(f"• 75th percentile (Q3): {advanced_stats['yearly_stats']['2024']['q3_fields']:.1f} fields",
                           bullet_style))
    story.append(Paragraph(
        f"• Interquartile Range (IQR): {advanced_stats['yearly_stats']['2024']['q3_fields'] - advanced_stats['yearly_stats']['2024']['q1_fields']:.1f} fields",
        bullet_style))
    ...
    story.append(Paragraph(
        "Each bar shows the percentage of companies with completed fields in each sector. Bars show realistic percentages (not capped at 100%). Sorted by percentage (descending).",
        normal_style))
    ...
    insights = [
        ...
        "8. Percentage Interpretation: Bars show percentage of companies with completed fields, not percentage of total possible fields completed."
    ]
    ...
    conclusions = [
        ...
        "10. Note: Percentage charts show realistic values - 100% would require all companies to have all 16 fields completed."
    ]
...
if __name__ == "__main__":
    try:
        ...
        print(f"\n✓ Key Fixes Applied:")
        print(f"  1. Fixed absolute chart errors by recreating figure instead of removing patches")
        print(f"  2. Percentage charts now show realistic values (not capped at 100%)")
        print(f"  3. All charts are sized consistently (16x8 inches)")
        print(f"  4. All graphs are sorted in descending order by bar height/percentage")
        print(f"  5. No distribution graph created (text analysis kept in PDF)")
